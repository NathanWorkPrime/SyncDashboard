...
app.get('/debug/sync-errors', function(req, res) {
  res.json({ success: true, count: debugErrors.length, errors: debugErrors });
});
    ...
    for (const rec of records) {
      ...
      try {
        ...
        if (wr.ok) {
          ...
          console.error(`  ❌ Failed FirmNo ${rec.FirmNo}: ${wr.status} - ${errText}`);
          debugErrors.push({ timestamp: new Date().toISOString(), table: 'Firms', id: rec.FirmNo, status: wr.status, body: errText });
          if (debugErrors.length > 500) debugErrors.shift();
          errors++;
        ...
        debugErrors.push({ timestamp: new Date().toISOString(), table: 'Firms', id: rec.FirmNo, status: 'EXCEPTION', body: err.message });
        if (debugErrors.length > 500) debugErrors.shift();
      }
    }
    ...
    if (customIds && customIds.length > 0) {
      ...
      customIds.forEach(id => {
        ...
        if (cleaned.includes('|')) {
          ...
          if (acc && firm) {
            conditions.push(`(ba.AccountNumber = '${acc}' AND org.Aff_FirmNo = '${firm}')`);
          } else if (acc) {
            conditions.push(`(ba.AccountNumber = '${acc}')`);
          }
        } else if (/^\d+$/.test(cleaned)) {
          idsList.push(cleaned);
        } else {
          conditions.push(`(ba.AccountNumber = '${cleaned}')`);
        }
      });
      ...
      for (const cond of allConditions) {
        const query = `
          SELECT 
              ba.Id,
              ba.AccountNumber,
              ba.BankLkp as BankCode,
              ba.AccountHolderName as Bank_Name,
              ba.BranchName as Branch,
              org.Aff_FirmNo as Firmno,
              ba.Frwk_CreatedTimestamp as Daterec,
              ba.Frwk_LastUpdatedTimestamp as trn_dte,
              ba.AFF_StatusLkp as InactiveFlag,
              ba.AccountTypeLkp,
              ba.DateClosed
        ...
        records = records.concat(res.recordset);
      }
    } else {
      const step1 = pool.request();
      step1.input('lastSyncTime', sql.DateTime2, new Date(originalWatermark));
      ...
      const query = `
        ...
        FROM dbo.Core_BankAccounts ba
        LEFT JOIN dbo.Core_Organisations org ON ba.Aff_FirmId = org.Id
        WHERE (ba.Frwk_LastUpdatedTimestamp > @lastSyncTime OR ba.Frwk_CreatedTimestamp > @lastSyncTime)
        ORDER BY ba.Frwk_LastUpdatedTimestamp ASC
      `;
      const res = await step1.query(query);
      records = res.recordset;
    }
    ...
    for (const rec of records) {
      ...
      try {
        ...
        if (wr.ok) {
          ...
          const errText = await wr.text();
          console.error(`  ❌ Failed AccountNumber ${rec.AccountNumber}: ${wr.status} - ${errText}`);
          debugErrors.push({ timestamp: new Date().toISOString(), table: 'Banks', id: rec.AccountNumber, status: wr.status, body: errText });
          if (debugErrors.length > 500) debugErrors.shift();
          errors++;
        ...
        debugErrors.push({ timestamp: new Date().toISOString(), table: 'Banks', id: rec.AccountNumber, status: 'EXCEPTION', body: err.message });
        if (debugErrors.length > 500) debugErrors.shift();
      }
    }
    ...
    if (customIds && customIds.length > 0) {
      ...
      const query = `
        SELECT 
            ...
            Aff_IsAdvocateWithFFC as FfcAdvocate,
    ...
    for (const rec of records) {
      ...
      try {
        const payload = {
          ...
          discriminator:         rec.Discriminator || null,
          pmt_status:            rec.PmtStatus !== null ? String(rec.PmtStatus) : null,
          province:              rec.Province !== null ? String(rec.Province) : null,
          red_flag:              rec.RedFlag !== null ? String(rec.RedFlag) : null,
          attorney:              rec.Attorney !== null ? String(rec.Attorney) : null,
          attorney_date:         rec.AttorneyDate ? rec.AttorneyDate.toISOString() : null,
          notary:                rec.Notary !== null ? String(rec.Notary) : null,
          notary_date:           rec.NotaryDate ? rec.NotaryDate.toISOString() : null,
          conveyancer:           rec.Conveyancer !== null ? String(rec.Conveyancer) : null,
          conveyancer_date:      rec.ConveyancerDate ? rec.ConveyancerDate.toISOString() : null,
          course_completed_date: rec.CourseCompletedDate ? rec.CourseCompletedDate.toISOString() : null,
          course_proof_date:     rec.CourseProofDate ? rec.CourseProofDate.toISOString() : null,
          date_inactive:         null,
          inactive_flag:         rec.InactiveFlag === true || rec.InactiveFlag === 1 ? "yes" : "no",
          physical_address:      rec.PhysicalAddress || null,
          postal_address:        rec.PostalAddress || null,
          username:              rec.Username || null,
          role:                  rec.Role !== null ? String(rec.Role) : null,
          advocate:              rec.Advocate !== null ? String(rec.Advocate) : null,
          advocate_date:         rec.AdvocateDate ? rec.AdvocateDate.toISOString() : null,
          ffc_advocate:          rec.FfcAdvocate !== null ? String(rec.FfcAdvocate) : null,
        ...
        if (wr.ok) {
          ...
          const errText = await wr.text();
          console.error(`  ❌ Failed MemNo ${rec.MemNo}: ${wr.status} - ${errText}`);
          errors++;
    ...
    for (const rec of records) {
      ...
      try {
        ...
        if (wr.ok) {
          ...
          const errText = await wr.text();
          console.error(`  ❌ Failed MemNo ${rec.memno}: ${wr.status} - ${errText}`);
          debugErrors.push({ timestamp: new Date().toISOString(), table: 'PractitionersAdm', id: rec.memno, status: wr.status, body: errText });
          if (debugErrors.length > 500) debugErrors.shift();
          errors++;
          failedIds.push(rec.memno);
        }
        ...
        debugErrors.push({ timestamp: new Date().toISOString(), table: 'PractitionersAdm', id: rec.memno, status: 'EXCEPTION', body: err.message });
        if (debugErrors.length > 500) debugErrors.shift();
      }
    }
    ...
    if (customIds && customIds.length > 0) {
      ...
      const query = `
        SELECT 
            ...
            cop.Frwk_InactiveFlag as inactive,
    ...
    for (const rec of records) {
      ...
      try {
        const payload = {
          ...
          discriminator:       rec.discriminator || null,
          inactive_flag:       rec.inactive !== null ? String(rec.inactive) : 'false',
        ...
        if (wr.ok) {
          ...
          const errText = await wr.text();
          console.error(`  ❌ Failed EmploymentID ${rec.id}: ${wr.status} - ${errText}`);
          debugErrors.push({ timestamp: new Date().toISOString(), table: 'EmploymentHistory', id: rec.id, status: wr.status, body: errText });
          if (debugErrors.length > 500) debugErrors.shift();
          errors++;
        ...
        debugErrors.push({ timestamp: new Date().toISOString(), table: 'EmploymentHistory', id: rec.id, status: 'EXCEPTION', body: err.message });
        if (debugErrors.length > 500) debugErrors.shift();
      }
    }
    ...
    if (customIds && customIds.length > 0) {
      ...
      if (guids.length > 0) {
        for (let i = 0; i < guids.length; i += chunkSize) {
          ...
          allConditions.push(`q.Id IN (${chunk.map(g => `'${g}'`).join(',')})`);
        }
      }
      if (ints.length > 0) {
        for (let i = 0; i < ints.length; i += chunkSize) {
          const chunk = ints.slice(i, i + chunkSize);
          allConditions.push(`fy.Id IN (${chunk.join(',')})`);
        }
      }
      
      if (allConditions.length === 0) {
        console.warn(`[doSyncProductionAudits] No valid custom IDs`);
        return { success: true, message: 'No valid custom IDs', synced: 0 };
      }
      ...
      for (const cond of allConditions) {
        const query = `
          SELECT 
              q.Id as ID,
              ISNULL(q.FirmNumber, ISNULL(fi.firmnumber, org.Aff_FirmNo)) as FIRMNO,
              fy.DueDate as DueDate,
              fy.ReceivedDate as Received,
              fy.IsAprroved as Approved,
              fy.IsQualified as Qualified,
              fy.ApprovedDate as ApprovedDate,
              fy.PeriodStartDate as PeriodStartDate,
              fy.PeriodEnddate as PeriodEnddate,
              fy.AuditFeesAmount as AuditFeesAmount,
              fy.GrossInterestAmount as GrossInterestAmount,
              fy.BankChargesAmount as BankChargesAmount,
              fy.NetInterestAmount as NetInterestAmount,
              fy.AuditorId as AuditorID,
              fy.AuditComplianceStatusLkp as AuditComplianceStatus,
              q.Frwk_Discriminator as Discriminator,
              q.Frwk_InactiveFlag as InactiveFlag,
              fy.Year as Year,
              fy.AuditTypeLkp as AuditType,
              fy.ReconApprovedById as UserAuditApproved,
              ISNULL(q.Frwk_LastUpdatedTimestamp, q.Frwk_CreatedTimestamp) as LastUpdated
          FROM dbo.Aff_FfcFirmQuestionnaires q
          LEFT JOIN dbo.AFF_FfcFirmInfoes fi ON q.FirmInfoId = fi.Id
          LEFT JOIN dbo.Aff_FirmFinancialYears fy ON fi.FirmId = fy.FirmId AND fi.Period = CAST(fy.Year AS NVARCHAR(50))
          LEFT JOIN dbo.Core_Organisations org ON fi.FirmId = org.Id
          WHERE ${cond}
        `;
        const res = await pool.request().query(query);
        records = records.concat(res.recordset);
      ...
      const query = `
        ...
        LEFT JOIN dbo.Aff_FirmFinancialYears fy ON fi.FirmId = fy.FirmId AND fi.Period = CAST(fy.Year AS NVARCHAR(50))
        LEFT JOIN dbo.Core_Organisations org ON fi.FirmId = org.Id
        WHERE (q.Frwk_LastUpdatedTimestamp > @lastSyncTime OR q.Frwk_CreatedTimestamp > @lastSyncTime)
        ORDER BY ISNULL(q.Frwk_LastUpdatedTimestamp, q.Frwk_CreatedTimestamp) ASC
      `;
      const res = await step1.query(query);
      records = res.recordset;
    }
    ...
    for (const rec of records) {
      ...
      try {
        const payload = {
          ...
          received_date:           rec.Received ? rec.Received.toISOString() : null,
          qualified:               rec.Qualified !== null ? String(rec.Qualified) : null,
          year:                    rec.Year !== null ? String(rec.Year) : null,
          audit_type:              rec.AuditType !== null ? String(rec.AuditType) : null,
          approved:                rec.Approved !== null ? String(rec.Approved) : null,
          approved_date:           rec.ApprovedDate ? rec.ApprovedDate.toISOString() : null,
          approved_by:             rec.UserAuditApproved !== null ? String(rec.UserAuditApproved) : null,
        ...
        if (wr.ok) {
          ...
          const errText = await wr.text();
          console.error(`  ❌ Failed ID ${rec.ID}: ${wr.status} - ${errText}`);
          debugErrors.push({ timestamp: new Date().toISOString(), table: 'Audits', id: rec.ID, status: wr.status, body: errText });
          if (debugErrors.length > 500) debugErrors.shift();
          errors++;
          failedIds.push(rec.ID);
        }
        ...
        debugErrors.push({ timestamp: new Date().toISOString(), table: 'Audits', id: rec.ID, status: 'EXCEPTION', body: err.message });
        if (debugErrors.length > 500) debugErrors.shift();
      }
    }
...
app.post("/sync-firms", async (req, res) => {
  ...
  if (source === 'production') {
    doSyncProductionFirms(topLimit, trigger, bubbleBase, devRun, isProduction, customIds)
      .catch(err => console.error("Error in background doSyncProductionFirms:", err));
  } else {
    doSyncFirms(topLimit, trigger, bubbleBase, devRun, isProduction, customIds)
      .catch(err => console.error("Error in background doSyncFirms:", err));
  }
...
app.post("/sync-banks", async (req, res) => {
  ...
  if (source === 'production') {
    doSyncProductionBanks(topLimit, trigger, bubbleBase, devRun, isProduction, customIds)
      .catch(err => console.error("Error in background doSyncProductionBanks:", err));
  } else {
    doSyncBanks(topLimit, trigger, bubbleBase, devRun, isProduction, customIds)
      .catch(err => console.error("Error in background doSyncBanks:", err));
  }
  res.json({ success: true, message: "Sync job started in background" });
});
...
app.post("/sync-practitioners", async (req, res) => {
  ...
  if (source === 'production') {
    doSyncProductionPractitioners(topLimit, trigger, bubbleBase, devRun, isProduction, customIds)
      .catch(err => console.error("Error in background doSyncProductionPractitioners:", err));
  } else {
    doSyncPractitioners(topLimit, trigger, bubbleBase, devRun, isProduction, customIds)
      .catch(err => console.error("Error in background doSyncPractitioners:", err));
  }
  res.json({ success: true, message: "Sync job started in background" });
});
...
app.post('/sync-employment-history', async (req, res) => {
  ...
  if (source === 'production') {
    doSyncProductionEmploymentHistory(topLimit, trigger, bubbleBase, devRun, isProduction, customIds)
      .catch(err => console.error("Error in background doSyncProductionEmploymentHistory:", err));
  } else {
    doSyncEmploymentHistory(topLimit, trigger, bubbleBase, devRun, isProduction, customIds)
      .catch(err => console.error("Error in background doSyncEmploymentHistory:", err));
  }
  res.json({ success: true, message: "Sync job started in background" });
});
...
app.post('/sync-audits', async (req, res) => {
  ...
  if (source === 'production') {
    doSyncProductionAudits(topLimit, trigger, bubbleBase, devRun, isProduction, customIds)
      .catch(err => console.error("Error in background doSyncProductionAudits:", err));
  } else {
    doSyncAudits(topLimit, trigger, bubbleBase, devRun, isProduction, customIds)
      .catch(err => console.error("Error in background doSyncAudits:", err));
  }
  res.json({ success: true, message: "Sync job started in background" });
});
...
app.post('/sync/stop', (req, res) => {
  const entity = req.body.entity || req.body.table;
